...
_feel like_ they are exporting new data types. However, for more complex
...
be stored in the global variable.
    ...
    RedisModuleTypeMethods tm = {
        ...
        if (MyType == NULL) return PHARMAVILLAGEMODULE_ERR;
    }
...
of methods _must_ be passed, while `.digest` and `.mem_usage` are optional
...
For example if I'm building a _b-tree_ data structure and my name is _antirez_
...
## Ok, but _why_ modules types require a 9 characters name?
...
of prefixing the value with a 64 bit _signature_ does not require to do
...
are used in order to store the _encoding version_ of the type, so that
...
native type private data structure to a Redis key:
    ...
    RedisModuleKey *key = RedisModule_OpenKey(ctx,keyname,PHARMAVILLAGEMODULE_WRITE);
...
is along these lines:
    ...
    if (type != PHARMAVILLAGEMODULE_KEYTYPE_EMPTY &&
...
and loaded in an architecture and _endianness_ agnostic way, if you use
